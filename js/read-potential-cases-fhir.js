...
(async () => {
  ...
  await fs.writeFile("social-dysfunction-potential-cases.csv", "patient-id,dob,codes,last-encounter\n");
  for(let patient in patients) {
    try {
      ...
      await fs.appendFile("social-dysfunction-potential-cases.csv", row);
    } catch(error) {
      console.log(error);
    }
  }
